shipmentEntry1Div = document.createElement("div");
shipmentEntry1Div.id = "shipmentEntry1";
shipmentEntry1Div.innerHTML = `
<div class="row">
    <div class="col-1 offset-md-3">
        <img class="d-block w-100" style="border-radius:10%" src="img/productos/hoodie/hoodieRosa.jpg">
    </div><div class="col-8"><div class="row">
    <div class="col-10">
        <p style="text-align:left"><b>Nike</b> Sudadera Rosa</p>
        <div class="row">
            <div class="col-4">
                <button class="productSize" style="width:auto; padding-left:10px; padding-right:10px; height:40px;">Ropa - L</button>
                <button class="productSize" style="width:auto; padding-left:10px; padding-right:10px; height:40px;">X 2</button>
            </div>
            <div class="col-1"><button class="productSize" style="width:auto; padding-left:10px; padding-right:10px; height:40px;">Total: </button></div>
            <div class="col-2 offset-md-1 productPriceYellow">$560</div>
        </div>
    </div>
    
          </div></div>
</div><br>
`;
document.getElementById("mainBody").appendChild(shipmentEntry1Div);